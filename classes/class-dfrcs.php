...
class Dfrcs {
	...
	private function query_amazon() {
		...
		foreach ( $fields as $field ) {
			...
			// Hardcode 'asin' field.
			$product['asin'] = $product['suid'];

			// Add additional model numbers &/or barcodes but only if this product has not been removed.
			if ( ! in_array( $product['suid'], (array) $this->removed ) ) {
				$this->source->build( $product );
			} else {
				$this->log( 'query_amazon_by_' . $field . '/removed', $product['suid'] );
			}
			...
			$this->products[ 'id_' . $id ]['_id']         = $product['suid'];
